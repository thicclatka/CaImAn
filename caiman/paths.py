...
def get_tempdir() -> str:
    ...
    if "CAIMAN_TEMP" in os.environ:
        if os.path.isdir(os.environ["CAIMAN_TEMP"]):
            ...
            logger.warning(
                f"CAIMAN_TEMP is set to nonexistent directory {os.environ['CAIMAN_TEMP']}. Ignoring"
            )
    temp_under_data = os.path.join(caiman_datadir(), "temp")
    if not os.path.isdir(temp_under_data):
        logger.warning(
            f"Default temporary dir {temp_under_data} does not exist, creating"
        )
...
def memmap_frames_filename(
    basename: str, dims: tuple, frames: int, order: str = "F"
) -> str:
...
def fname_derived_presuffix(
    basename: str, addition: str, swapsuffix: str = None
) -> str:
...
def decode_mmap_filename_dict(basename: str) -> dict:
    ...
    for field in ["d1", "d2", "d3", "order", "frames"]:
        ...
        for i in range(
            len(fpart) - 1, -1, -1
        ):  # Step backwards through the list; defensive programming
            if field == fpart[i]:
                if field == "order":  # a string
                    ret[field] = fpart[
                        i + 1
                    ]  # Assume no filenames will be constructed to end with a key and not a value
                else:  # numeric
                    ret[field] = int(
                        fpart[i + 1]
                    )  # Assume no filenames will be constructed to end with a key and not a value
...
def generate_fname_tot(
    ...
    ret = "_".join(
        [base_name, "d1", str(d1), "d2", str(d2), "d3", str(d3), "order", order]
    )
